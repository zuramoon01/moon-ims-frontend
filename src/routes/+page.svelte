<script lang="ts">
	...
	import { Button, Checkbox, Icon, Link } from "$lib/ui";
...
<main class="flex w-[calc(100%_-_theme(spacing.sidebar))] flex-col items-start">
	<section class="flex w-full flex-col items-start px-8">
		...
		{#if config && config.total > 0}
			...
			>
				<div
					class="shadow-border-b flex h-8 w-full px-4 font-semibold text-accent-950"
				>
					<div
						class="shadow-border-r flex w-8 shrink-0 items-center justify-center"
					>
						<Checkbox />
					</div>
					...
					{#each productTableTitles as { id, text, classes } (id)}
						...
						<div
							class={clsx(
								"shadow-border-r flex cursor-pointer items-center justify-between px-4",
								"hover:bg-accent-100",
								classes,
							)}
							on:click={() => {
				...
				{#each $productStore as { id, name, quantity, buyPriceInRupiah, totalBuyPriceInRupiah, sellPriceInRupiah, totalSellPriceInRupiah } (id)}
					<div
						class={clsx(
							"shadow-border-b flex min-h-8 w-full px-4",
							"hover:bg-accent-100",
						)}
					>
						<div
						...
						<div
							class="shadow-border-r flex w-[calc(100%_-_(32px_+_160px_+_240px_+_240px_+_200px_+_200px_+_80px))] min-w-[200px] items-center px-4 py-1"
						>
							<span class="text-sm">{name}</span>
						</div>

						<div
							class="shadow-border-r flex w-[160px] shrink-0 items-center px-4 py-1"
						>
							<span>{quantity}</span>
						</div>

						<div
							class="shadow-border-r flex w-[240px] shrink-0 items-center px-4 py-1"
						>
							<span>{buyPriceInRupiah}</span>
						</div>

						<div
							class="shadow-border-r flex w-[200px] shrink-0 items-center px-4 py-1"
						>
			...
			>
				<div class="flex items-center gap-2">
					<input
						type="text"
						value={config.limit}
						class="shadow-border flex h-6 w-10 items-center justify-center rounded bg-accent-50 px-2 text-center text-sm font-medium leading-none"
						on:keyup={(e) => {
							if (e.key === "Enter") {
